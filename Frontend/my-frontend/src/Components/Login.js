...
import {jwtDecode} from "jwt-decode"; // Corrected import for decoding JWT
  ...
  const handleSubmit = (e) => {
    ...
    axios
      ...
      .then((response) => {
        ...
        // Store JWT token in sessionStorage after successful login
        const token = response.data.token;
        sessionStorage.setItem('token', token);

        // Decode the token to get the username (or email) from the payload
        const decodedToken = jwtDecode(token);
        const username = decodedToken.sub; // Adjust based on your token structure
